# lagou
该项目是基于拉勾网数据实现的PC端求职网站，它可以通过搜索 和 选择城市 搜寻需要的工作信息和公司信息，也可以展示详细的工作和公司信息，包括工作薪水，公司地址标记等等
...
职位描述和公司介绍的大段文字处理问题
```
格式不统一，很难进行分割，导致布局乱七八糟
```
刷新页面数据丢失问题
```
因为http的无状态性，通过本地保存当前工作的id信息，使得在刷新或者回退时依然可以获取到信息。
```
后台数据处理问题

```
虽然是获取单个工作的数据，但是不正确处理数据，会使得页面难以简单正确的显示

通过v-for的方式和通过data初始化的方式

通过v-for渲染，会等待数据获取成功后再执行
而通过data初始变量的方式，则可能出现数据还未获取，就渲染的情况，那么结果是报错
```
地址标注
```
在公司详情页，通过万德的开放接口，查询并标注公司的地址。
```
> 盒子定位被覆盖时z-index没有效果
